...
args
.option('path', 'The root directory in which to create the Flarum extension', process.cwd(), p => path.resolve(p))
  ...
  return inquirer.prompt([
    {
      name: 'packageName',
      message: 'Package:',
      validate: (s) => /^([a-zA-Z-]{2,})\/([a-zA-Z-]{2,})$/.test(s.trim()) || 'Invalid package name format, author/extension-name',
    ...
    },
    {
      ...
      source(o, s) {
        ...
        return Promise.resolve(s ? licenseList.filter(l => regex.test(l)) : licenseList)
      },
    },
    {
      name: 'extensionName',
